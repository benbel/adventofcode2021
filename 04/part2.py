...
def tableau_rank(tableau, drawn_numbers):
    for i in range(len(drawn_numbers)):
        drawn_numbers_so_far = drawn_numbers[0:i]
    
        if check_tableau(tableau, drawn_numbers_so_far):
            unmarked_numbers = [nombre for ligne in tableau for nombre in ligne if nombre not in drawn_numbers_so_far]
...
ranks_and_scores = [tableau_rank(tableau, drawn_numbers) for tableau in tableaux]
max_rank = max(rank for (rank, score) in ranks_and_scores)

rank, score = [result for result in ranks_and_scores if result[0] == max_rank].pop()
print(score)
